...
def calculate_and_plot_ACF (df: pd.DataFrame):
    df = df.drop(columns=['Date'])
    column_count = len(df.columns)
    fig, axs = plt.subplots(nrows=math.floor(math.sqrt(column_count)),ncols=math.ceil(math.sqrt(column_count)))
    

    for i, column in enumerate(df.columns):
        row_index = i // math.ceil(column_count / (math.floor(math.sqrt(column_count))))
        col_index = i %  math.ceil(column_count / (math.floor(math.sqrt(column_count)))) 

        plot_acf(pd.Series(df[column]), ax=axs[row_index, col_index] , title=f'ACF for {column}', lags = 80)
